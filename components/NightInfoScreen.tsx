...
type CardType = 'you_are' | 'this_player_is' | 'selected_you';
  ...
  return (
    ...
    >
      ...
      <p
        ...
        style={{
          fontSize: 'clamp(16px, 3.5vmin, 32px)',
      ...
      {/* Ability text — only shown for "You are" so the player can read their role */}
      {cardType === 'you_are' && (
        <p
    ...
    >
      ...
      >
        ...
        <button
          ...
          style={{
            background: selectedRole ? 'rgba(99,102,241,0.2)' : 'transparent',
            border: `1px solid ${selectedRole ? '#6366f1' : 'transparent'}`,
            color: selectedRole ? '#a5b4fc' : 'transparent',
          }}
        >
          Show
        </button>
      ...
      <div className="px-4 pt-3 pb-1 flex-shrink-0 flex flex-col gap-2">
        {(['you_are', 'this_player_is', 'selected_you'] as const).map(type => (
          <button
            key={type}
